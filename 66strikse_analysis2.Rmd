...
```{r}
# Count occurrences of each ActionType
action_summary <- df %>%
...
dfStrike <- df %>%
  filter(ActionType %in% c("wildcat", "strike","strike [assumed authorized]"))


sum_on_strike <- sum(dfStrike$OnStrike, na.rm = TRUE)
sum_days_lost_min <- sum(dfStrike$WorkerDaysLostMin, na.rm = TRUE)
sum_days_lost_max<-sum(dfStrike$WorkerDaysLostMax,na.rm=TRUE)
